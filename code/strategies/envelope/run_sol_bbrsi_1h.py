...
params = {
    ...
    # RSI entry triggers
    'rsi_period': 14,
    'rsi_long_max': 42,      # allow longs when RSI <= this (oversold)
    'rsi_short_min': 58,     # allow shorts when RSI >= this (overbought)

    # Regime filter (avoid trending markets for mean reversion)
    'adx_period': 14,
    'adx_max':20,           # only mean reversion when ADX < 18

    # ATR-based risk (recommended for 1H)
    'atr_period': 14,
    'stop_atr_mult': 1.7,   # stop distance = ATR * mult
