...
params = {
    ...
    'ema_slow': 200,
    'adx_period': 14,
    'adx_min': 20,              # trade only when ADX >= 20
    ...
    # Entry behavior
    'trigger_price_delta': 0.002,  # 0.2% trigger distance for 1H
}
...
def main():
    ...
    if adx_v < params["adx_min"]:
        print(f"{datetime.now().strftime('%H:%M:%S')}: ADX={adx_v:.1f} < {params['adx_min']}, skip")
        return
    ...
    if params["use_longs"] and ema_fast > ema_slow and close_v <= ema_fast:
    ...
    elif params["use_shorts"] and ema_fast < ema_slow and close_v >= ema_fast:
