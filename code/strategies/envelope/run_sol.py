...
params = {
    ...
    'timeframe': '1h',                       # ✅ CHANGED (was 1h)
    'margin_mode': 'isolated',               # 'cross'
    'balance_fraction': 1,
    'leverage': 2,                           # ✅ as per your deploy
    'average_type': 'EMA',                   # ✅ CHANGED (was SMA)
    'average_period': 6,                    # ✅ CHANGED (was 6)
    'envelopes': [0.05, 0.08, 0.11],               # ✅ CHANGED (was [0.07,0.11,0.14])
    'stop_loss_pct': 0.06,                   # ✅ CHANGED (was 0.35)
    'price_jump_pct': 0.3,
    'position_size_percentage': 25,          # ✅ CHANGED (was 40) AND NOW USED PROPERLY
    'mode': "long",
    'trend_filter': True,
    'trend_filter_period': 150,
    'trend_filter_directional': True,
    'trend_buffer_pct': 0.002,          # 0.2% neutral zone around trend EMA
    'max_extension_pct': 0.01,          # block shorts in strong upward momentum
    'use_longs': True,
    'use_shorts': False,
...
# ✅ CHANGED — trigger delta tuned for 1h (slightly wider than 1h to avoid noise triggers)
# You can adjust 0.008–0.012 based on fills/false triggers.
trigger_price_delta = 0.003
...
# ✅ NEW — Trend permission (DIRECTIONAL) at last CLOSED candle
...
if params.get("trend_filter", False):
    ...
    if (price != price) or (avg != avg) or (trn != trn):
        ...
        buffer = float(params.get("trend_buffer_pct", 0.0) or 0.0)

        if params.get("trend_filter_directional", True):
            # Regime filter with neutral zone (hysteresis) around trend EMA
            ...
            # Legacy behavior: average vs trend_ema
            allow_long_by_trend = avg > trn
            allow_short_by_trend = avg < trn

        # Anti-momentum / extension filter (SHORTS only; keeps envelope longs intact)
        max_ext = params.get("max_extension_pct", None)
        if max_ext is not None and allow_short_by_trend:
            max_ext = float(max_ext)
            if price > avg * (1 + max_ext):
                allow_short_by_trend = False

    print(
        f"{datetime.now().strftime('%H:%M:%S')}: trend_filter EMA{params['trend_filter_period']} "
        f"price={price:.4f} avg(EMA{params['average_period']})={avg:.4f} trend={trn:.4f} "
...
resume_price = data['average'].iloc[signal_idx]
...
if open_position:
    ...
    bitget.place_trigger_market_order(
        ...
        side=close_side,
        amount=amount,
        trigger_price=data['average'].iloc[signal_idx],
        reduce=True,
        print_error=True,
    )
    ...
    print(
        ...
        f"exit price {data['average'].iloc[signal_idx]}, sl price {stop_loss_price}"
    )
else:
    info = {
        "status": "ok_to_trade",
