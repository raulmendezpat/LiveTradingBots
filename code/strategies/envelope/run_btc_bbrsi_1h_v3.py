...
params = {
    ...
    # RSI entry triggers
    'rsi_period': 14,
    'rsi_long_max': 33,      # allow longs when RSI <= this (oversold)
    'rsi_short_min': 67,     # allow shorts when RSI >= this (overbought)

    # Regime filter (avoid trending markets for mean reversion)
    'adx_period': 14,
    'adx_max': 16,           # only mean reversion when ADX < 18

    # ATR-based risk (recommended for 1H)
    'atr_period': 14,
    'stop_atr_mult': 1.3,   # stop distance = ATR * mult
    'tp_to_basis': True,      # TP at BB basis (middle band)

    # Entry order behavior (uses trigger-limit like your envelope bot)
    'trigger_price_delta': 0.003,  # closer for 1H
}
...
def main():
    ...
    if adx_v >= params["adx_max"]:
        print(f"{datetime.now().strftime('%H:%M:%S')}: ADX={adx_v:.2f} >= {params['adx_max']}, skip (trend regime)")
        return
