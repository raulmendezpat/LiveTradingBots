#!/usr/bin/env python3
"""
optimize_regime_switch_params_v5_1_balanced_search.py
...
Usage:
python3 code/strategies/envelope/optimize_regime_switch_params_v5_1_balanced_search.py \
...
def penalty_balance(trend_share: float, target: float, band: float) -> float:
    """Penalty in (0,1] encouraging trend_share near target."""
    band = float(band)
    if band <= 0:
        return 1.0
    dev = abs(float(trend_share) - float(target))
    if dev <= band:
        return 1.0
    return max(0.0, min(1.0, band / max(dev, 1e-12)))
...
def main():
    ...
    # v5.1: scoring + balanced-search controls
    ap.add_argument("--test-weight", type=float, default=0.85)
    ap.add_argument("--train-weight", type=float, default=0.15)
    ap.add_argument("--overfit-lambda", type=float, default=0.25)
    ap.add_argument("--trend-target", type=float, default=0.45)
    ap.add_argument("--trend-band", type=float, default=0.10)
    ap.add_argument("--penalty-balance-power", type=float, default=4.0)

    # v5.1: expanded search ranges (used by --balanced-search)
    ap.add_argument("--adx-off-min", type=float, default=18.0)
    ap.add_argument("--adx-off-max", type=float, default=30.0)
    ap.add_argument("--adx-on-max",  type=float, default=45.0)
    ap.add_argument("--vol-off-min", type=float, default=0.0060)
    ap.add_argument("--vol-off-max", type=float, default=0.0150)
    ap.add_argument("--vol-on-max",  type=float, default=0.0200)
    ap.add_argument("--balanced-search", action="store_true")
    ...
    for i in range(args.trials):
        ...
        # v5.1: optional biased sampling to reduce 'always TREND' for OR-enter/AND-exit
        if args.balanced_search and args.mode == "adx_or_enter_and_exit":
            adx_on  = rng.triangular(max(adx_off + 0.5, 21.0), float(args.adx_on_max), float(args.adx_on_max))
            vol_on  = rng.triangular(max(vol_off + 0.0008, 0.0082), float(args.vol_on_max), float(args.vol_on_max))
            adx_off = rng.triangular(float(args.adx_off_min), float(args.adx_off_max), float(args.adx_off_max))
            vol_off = rng.triangular(float(args.vol_off_min), float(args.vol_off_max), float(args.vol_off_max))
        ...
        # v5.1 scoring override: prioritise TEST, penalise overfit, encourage balanced regime
        tw = float(args.test_weight); rw = float(args.train_weight)
        s = tw + rw
        if s <= 0:
            tw, rw, s = 0.85, 0.15, 1.0
        tw, rw = tw / s, rw / s

        weighted = (tw * float(score_test)) + (rw * float(score_train))
        weighted -= float(args.overfit_lambda) * abs(float(score_train) - float(score_test))

        pen_bal_test  = penalty_balance(test_trend_share,  args.trend_target, args.trend_band)
        pen_bal_train = penalty_balance(train_trend_share, args.trend_target, args.trend_band)
        pen_bal = (pen_bal_test * pen_bal_train) ** float(args.penalty_balance_power)

        final = weighted * soft_mult * pen_bal
...
# Track best trial
        ...
        rows.append({
            ...
            "pen_balance_test": float(pen_bal_test),
            "pen_balance_train": float(pen_bal_train),
            "adx_on": float(adx_on),
